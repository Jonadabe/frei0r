...
public:
  ...
  void register_param(f0r_param_color &p_loc, const std::string &name,
                      const std::string &desc) {
    param_ptrs.push_back(&p_loc);
    s_params.push_back(param_info(name, desc, F0R_PARAM_COLOR));
  }

  void register_param(double &p_loc, const std::string &name,
                      const std::string &desc) {
    param_ptrs.push_back(&p_loc);
    s_params.push_back(param_info(name, desc, F0R_PARAM_DOUBLE));
  }

  void register_param(bool &p_loc, const std::string &name,
                      const std::string &desc) {
    param_ptrs.push_back(&p_loc);
    s_params.push_back(param_info(name, desc, F0R_PARAM_BOOL));
  }

  void register_param(f0r_param_position &p_loc, const std::string &name,
                      const std::string &desc) {
    param_ptrs.push_back(&p_loc);
    s_params.push_back(param_info(name, desc, F0R_PARAM_POSITION));
  }

  void register_param(std::string &p_loc, const std::string &name,
                      const std::string &desc) {
    param_ptrs.push_back(&p_loc);
  ...
  void get_param_value(f0r_param_t param, int param_index) {
    ...
    case F0R_PARAM_STRING:
      *static_cast<f0r_param_string *>(param) = const_cast<f0r_param_string>(
          static_cast<std::string *>(ptr)->c_str());
      break;
    }
  }
  ...
  void set_param_value(f0r_param_t param, int param_index) {
    ...
    switch (s_params[param_index].m_type) {
    case F0R_PARAM_BOOL:
      *static_cast<bool *>(ptr) = (*static_cast<f0r_param_bool *>(param) > 0.5);
      break;
    case F0R_PARAM_DOUBLE:
      *static_cast<f0r_param_double *>(ptr) =
          *static_cast<f0r_param_double *>(param);
  ...
  virtual void update(double time, uint32_t *out, const uint32_t *in1,
                      const uint32_t *in2, const uint32_t *in3) = 0;
...
public:
  construct(const std::string &name, const std::string &explanation,
            const std::string &author, const int &major_version,
...
void f0r_update2(f0r_instance_t instance, double time, const uint32_t *inframe1,
                 ...
                 uint32_t *outframe) {
  static_cast<frei0r::fx *>(instance)->update(time, outframe, inframe1,
                                              inframe2, inframe3);
}
