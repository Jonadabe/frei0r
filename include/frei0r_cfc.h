/* cfc.h
 * uchar->float and float->uchar conversion for packed RGBA video with flexible
 * gamma correction
 *
...
//--------------------------------------------------------
// the following gamma functions need not be speed optimized, as they are only
...
// a = input, should be in the 0.0 to 1.0 range
static inline float gamma_plain_f(float a, float g) { return powf(a, 1.0 / g); }

//--------------------------------------------------------
// plain gamma (power function) backward (gamma to linear)
// a = input, should be in the 0.0 to 1.0 range
static inline float gamma_plain_b(float a, float g) { return powf(a, g); }

//--------------------------------------------------------
// dummy function for linear tables (no gamma)
// g = gamma value
static inline float gamma_none(float a) { return a; }
...
//--------------------------------------------------------
// convert from packed uchar RGBA to packed float RGBA
...
static inline void RGBA8_2_float(const uint32_t *in, float_rgba *out, int w,
                                 int h, float *tab, float *atab) {
  int i;
...
// w, h are width and height of the image
// tab = table used for RGB conversion
static inline void RGB8_2_float(const uint32_t *in, float_rgba *out, int w,
                                int h, float *tab) {
  int i;
  uint8_t *cin = (uint8_t *)in;
...
// tab = table used for RGB conversion
// atab = table used for alpha conversion (usually linear)
static inline void float_2_RGBA8(const float_rgba *in, uint32_t *out, int w,
                                 int h, uint8_t *tab, uint8_t *atab) {
  int i;
...
// covert RGB only, SKIP ALPHA
// tab = table used for RGB conversion
static inline void float_2_RGB8(const float_rgba *in, uint32_t *out, int w,
                                int h, uint8_t *tab) {
  int i;
  uint8_t *cout = (uint8_t *)out;
...
// there is no function for single uint8 to float, because that is just a simple
// table lookup
//
// because float_2_uint8() can be called immediately after the input value has
// been created, type punning has to be done properly, with a union
