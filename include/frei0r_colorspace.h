 ...
 *
 * The arguments are pointers to int representing channel values in the RGB
 * colorspace, and the values pointed to are all in the range [0, 255].
 *
 * The function changes the arguments to point to the HSV value corresponding,
 * with the returned values in the following ranges: H [0, 360], S [0, 255], V
 * [0, 255].
 **/
inline void rgb_to_hsv_int(int *red /* returns hue        */,
 ...
 *
 * The function changes the arguments to point to the RGB value corresponding,
 * with the returned values all in the range [0, 255].
 **/
inline void hsv_to_rgb_int(int *hue /* returns red        */,
 ...
 * colorspace, and the values pointed to are all in the range [0, 255].
 *
 * The function changes the arguments to point to the corresponding HLS value
 * with the values pointed to in the following ranges:  H [0, 360], L [0, 255],
 * S [0, 255].
 **/
inline void rgb_to_hsl_int(unsigned int *red /* returns red        */,
 ...
 * with the returned values all in the range [0, 255].
 **/
inline void hsl_to_rgb_int(unsigned int *hue /* returns red        */,
 ...
 * @pullout: the percentage of black to pull out (0-100); returns the black
 *           value (0-255)
 *
 * Does a naive conversion from RGB to CMYK colorspace. A simple formula that
 * doesn't take any color-profiles into account is used. The amount of black
 * pullout how can be controlled via the @pullout parameter. A @pullout value of
 * 0 makes this a conversion to CMY. A value of 100 causes the maximum amount of
 * black to be pulled out.
 **/
inline void gimp_rgb_to_cmyk_int(int *red, int *green, int *blue,
                                 int *pullout) {
  int c, m, y;
 ...
 *
 * Does a naive conversion from CMYK to RGB colorspace. A simple formula that
 * doesn't take any color-profiles into account is used.
 **/
