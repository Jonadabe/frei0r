...
void f0r_get_param_info(f0r_param_info_t *info, int param_index) {
  ...
  case 0:
    ...
    info->explanation = "Number of rows in the image grid. Input range 0 - 1 "
                        "is interpreted as range 1 - 20";
    break;
  case 1:
    info->name = "columns";
    info->type = F0R_PARAM_DOUBLE;
    info->explanation = "Number of columns in the image grid. Input range 0 - "
                        "1 is interpreted as range 1 - 20";
    break;
  }
}

f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
  cairo_imagegrid_instance_t *inst =
      (cairo_imagegrid_instance_t *)calloc(1, sizeof(*inst));
  inst->width = width;
  inst->height = height;
...
void draw_grid(cairo_imagegrid_instance_t *inst, unsigned char *dst,
  ...
  cairo_surface_t *dest_image = cairo_image_surface_create_for_data(
      (unsigned char *)dst, CAIRO_FORMAT_ARGB32, w, h, stride);
  cairo_t *cr = cairo_create(dest_image);

  cairo_surface_t *image = cairo_image_surface_create_for_data(
      (unsigned char *)src, CAIRO_FORMAT_ARGB32, w, h, stride);
