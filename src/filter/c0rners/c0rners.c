...
void cetverokotnik4(int wi, int hi, int wo, int ho, tocka2 vog[], int str,
                    float strx, float stry, float *map) {
  double a, b, c, d, e, f, g, h, a2, b2, c2, u, v, aa, bb, de, sde, v1, v2, u1,
      u2;
  tocka2 T;
  int x, y;
  float kx, ky, k1, k2;
  ...
  de = 0.0;
  v1 = 1000.0;
  v2 = 1000.0; // da compiler ne jamra
  ...
  for (y = 0; y < ho; y++) {
    for (x = 0; x < wo; x++) {
      ...
      if ((u >= 0.0) && (u <= 1.0) && (v >= 0.0) &&
          (v <= 1.0)) { // ce smo znotraj orig slike
        map[2 * (y * wo + x)] = u * (wi - 1);
...
void trikotnik1(int wi, int hi, int wo, int ho, tocka2 vog[], tocka2 R,
                tocka2 S, premica2 p12, premica2 p23, premica2 p34,
                premica2 p41, int t12, int t23, int str, float strx, float stry,
                float *map) {
  int x, y;
  tocka2 T, A, B;
  premica2 p5, p6;
...
void make_alphamap(unsigned char *amap, tocka2 vog[], int wo, int ho,
                   float *map, float feath, int nots[]) {
...
int geom4c_b(int wi, int hi, int wo, int ho, tocka2 vog[], int str, float strx,
             float stry, float *map, int nots[]) {
  premica2 p12, p23, p34, p41;
  tocka2 R, S;
  ...
  if ((t12 + t34 + t41 + t23) < -19) { // vec kot dve sovpadata
    // daj tu fill with background??
    return 0;
  }
  if (((vog[0].x == vog[2].x) && (vog[0].y == vog[2].y)) ||
      ((vog[1].x == vog[3].x) &&
       (vog[1].y == vog[3].y))) { // sovpadata dve diagonalni tocki
    // daj tu fill with background??
    return 0;
  }
  ...
  if (p1 == -1) // vzporedni   1-2 in 3-4
  {
    ...
    {
      S.y = 1.0E9;
      S.x = -(p12.b * S.y + p12.c) / p12.a;
    } else {
      S.x = 1.0E9;
      S.y = -(p12.a * S.x + p12.c) / p12.b;
    }
  }
  if (p2 == -1) // vzporedni   2-3 in 4-1
  {
    ...
    {
      R.y = 1.0E9;
      R.x = -(p41.b * R.y + p41.c) / p41.a;
    } else {
      R.x = 1.0E9;
      R.y = -(p41.a * R.x + p41.c) / p41.b;
    }
  }
  ...
  case 0: // should never come to here...
    break;
  case 1: // triangle
    trikotnik1(wi, hi, wo, ho, vog, R, S, p12, p23, p34, p41, t12, t23, str,
               strx, stry, map);
    break;
  case 2: // paralelogram
...
interpp set_intp(inst p) {
  ...
  case 0:
    return interpNN_b32; // nearest neighbor
  case 1:
    return interpBL_b32; // bilinear
  case 2:
    return interpBC_b32; // bicubic smooth
  case 3:
    return interpBC2_b32; // bicibic sharp
  case 4:
    return interpSP4_b32; // spline 4x4
  case 5:
    return interpSP6_b32; // spline 6x6
  case 6:
    return interpSC16_b32; // lanczos 8x8
  default:
    return NULL;
  }
}
...
//-----------------------------------------------------
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t parm,
                         int param_index) {
  inst *p;
  double tmpf;
  int chg;
...
void f0r_get_param_value(f0r_instance_t instance, f0r_param_t param,
  ...
  case 10: // Stretch Y
    ...
    *((double *)param) = tmpf;
    break;
  case 11: // Interpolation
    *((double *)param) = map_value_backward(
        p->intp, 0.0, 6.0); //!!!!!! 6.999 ???? tudi v defish!!!!
    break;
  case 12: // Transparent Background
...
void f0r_update(f0r_instance_t instance, double time, const uint32_t *inframe,
  ...
  if (EQUIVALENT_FLOATS(p->x1, 0.333333f) &&
      ...
      (!p->stretchON || (EQUIVALENT_FLOATS(p->stretchx, 0.5f) &&
  ...
  if (p->mapIsDirty) {
    ...
    geom4c_b(p->w, p->h, p->w, p->h, vog, p->stretchON, p->stretchx,
             p->stretchy, p->map, nots);
  ...
  remap32(p->w, p->h, p->w, p->h, (unsigned char *)inframe,
          (unsigned char *)outframe, p->map, bkgr, p->interp);
