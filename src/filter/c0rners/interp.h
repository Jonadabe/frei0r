...
typedef int (*interpp)(unsigned char *, int, int, float, float,
                       unsigned char *);
...
void remap(int wi, int hi, int wo, int ho, unsigned char *vhs,
           unsigned char *izs, float *map, unsigned char bgc, interpp interp) {
  int i, j;
  float x, y;
...
//  bgc = background color
//  interp = kazalec na interpolacijsko funkcijo
void remap32(int wi, int hi, int wo, int ho, unsigned char *vhs,
             unsigned char *izs, float *map, uint32_t bgc, interpp interp) {
  int i, j;
  float x, y;
...
int interpNNpr_b(unsigned char *sl, int w, int h, float x, float y,
                 unsigned char *v) {
...
//	x,y tocka, za katero izracuna interpolirano vrednost
//	*v interpolirana vrednost
int interpNN_b(unsigned char *sl, int w, int h, float x, float y,
               unsigned char *v) {
#ifdef TEST_XY_LIMITS
  if ((x < 0) || (x > w) || (y < 0) || (y > h))
    return -1;
...
//	x,y tocka, za katero izracuna interpolirano vrednost
//	*v interpolirana vrednost
int interpNN_b32(unsigned char *sl, int w, int h, float x, float y,
                 unsigned char *v) {
#ifdef TEST_XY_LIMITS
  if ((x < 0) || (x > w) || (y < 0) || (y > h))
    return -1;
...
//	x,y tocka, za katero izracuna interpolirano vrednost
//	*v interpolirana vrednost
int interpBL_b(unsigned char *sl, int w, int h, float x, float y,
               unsigned char *v) {
  int m, n, k, l;
  float a, b;
...
// bilinearna interpolacija
// za byte (char) vrednosti  v packed color 32 bitnem formatu
int interpBL_b32(unsigned char *sl, int w, int h, float x, float y,
                 unsigned char *v) {
...
//	x,y tocka, za katero izracuna interpolirano vrednost
//	*v interpolirana vrednost
int interpBC_b(unsigned char *sl, int w, int h, float x, float y,
               unsigned char *v) {
  int i, j, l, m, n;
  float k;
  ...
  if (p[3] < 0.0)
    p[3] = 0.0; // printf("p=%f ",p[3]);
  if (p[3] > 256.0)
    p[3] = 255.0; // printf("p=%f ",p[3]);
...
// bikubicna interpolacija  "smooth"
// za byte (char) vrednosti  v packed color 32 bitnem formatu
int interpBC_b32(unsigned char *sl, int w, int h, float x, float y,
                 unsigned char *v) {
  int i, j, b, l, m, n;
  float k;
  float p[4], p1[4], p2[4], p3[4], p4[4];
...
int interpBC2_b(unsigned char *sl, int w, int h, float x, float y,
                unsigned char *v) {
  int i, k, l, m, n;
  ...
  xx = y - n;
  wy[0] = (-0.75 * (xx - 5.0) * xx - 6.0) * xx + 3.0;
  xx = xx - 1.0;
  wy[1] = (1.25 * xx - 2.25) * xx * xx + 1.0;
  xx = 1.0 - xx;
  wy[2] = (1.25 * xx - 2.25) * xx * xx + 1.0;
  xx = xx + 1.0;
  wy[3] = (-0.75 * (xx - 5.0) * xx - 6.0) * xx + 3.0;
  // se po x
  xx = x - m;
  wx[0] = (-0.75 * (xx - 5.0) * xx - 6.0) * xx + 3.0;
  xx = xx - 1.0;
  wx[1] = (1.25 * xx - 2.25) * xx * xx + 1.0;
  xx = 1.0 - xx;
  wx[2] = (1.25 * xx - 2.25) * xx * xx + 1.0;
  xx = xx + 1.0;
  wx[3] = (-0.75 * (xx - 5.0) * xx - 6.0) * xx + 3.0;
...
//!!! ODKOD SUM???  (ze po eni rotaciji v interp_test !!)
//!!! v defish tega suma ni???
int interpBC2_b32(unsigned char *sl, int w, int h, float x, float y,
                  unsigned char *v) {
  int b, i, k, l, m, n, u;
  float pp, p[4], wx[4], wy[4], xx;
...
//	x,y tocka, za katero izracuna interpolirano vrednost
//	*v interpolirana vrednost
int interpSP4_b(unsigned char *sl, int w, int h, float x, float y,
                unsigned char *v) {
  int i, j, m, n;
  float pp, p[4], wx[4], wy[4], xx;
  ...
  // najprej po y (stiri stolpce)
  xx = y - n;
  wy[0] = ((-0.333333 * (xx - 1.0) + 0.8) * (xx - 1.0) - 0.466667) * (xx - 1.0);
  xx = xx - 1.0;
  wy[1] = ((xx - 1.8) * xx - 0.2) * xx + 1.0;
  xx = 1.0 - xx;
  wy[2] = ((xx - 1.8) * xx - 0.2) * xx + 1.0;
  xx = xx + 1.0;
  wy[3] = ((-0.333333 * (xx - 1.0) + 0.8) * (xx - 1.0) - 0.466667) * (xx - 1.0);
  // se po x
  xx = x - m;
  wx[0] = ((-0.333333 * (xx - 1.0) + 0.8) * (xx - 1.0) - 0.466667) * (xx - 1.0);
  xx = xx - 1.0;
  wx[1] = ((xx - 1.8) * xx - 0.2) * xx + 1.0;
  xx = 1.0 - xx;
  wx[2] = ((xx - 1.8) * xx - 0.2) * xx + 1.0;
  xx = xx + 1.0;
  wx[3] = ((-0.333333 * (xx - 1.0) + 0.8) * (xx - 1.0) - 0.466667) * (xx - 1.0);
...
// spline 4x4 interpolacija
// za byte (char) vrednosti  v packed color 32 bitnem formatu
int interpSP4_b32(unsigned char *sl, int w, int h, float x, float y,
                  unsigned char *v) {
  int i, j, m, n, b;
  float pp, p[4], wx[4], wy[4], xx;
...
int interpSP6_b(unsigned char *sl, int w, int h, float x, float y,
                unsigned char *v) {
  int i, j, m, n;
  ...
  xx = y - n;
  wy[0] =
      ((0.090909 * (xx - 2.0) - 0.215311) * (xx - 2.0) + 0.124402) * (xx - 2.0);
  xx = xx - 1.0;
  wy[1] = ((-0.545455 * (xx - 1.0) + 1.291866) * (xx - 1.0) - 0.746411) *
          (xx - 1.0);
  xx = xx - 1.0;
  ...
  xx = xx + 1.0;
  wy[4] = ((-0.545455 * (xx - 1.0) + 1.291866) * (xx - 1.0) - 0.746411) *
          (xx - 1.0);
  xx = xx + 1.0;
  wy[5] =
      ((0.090909 * (xx - 2.0) - 0.215311) * (xx - 2.0) + 0.124402) * (xx - 2.0);
  ...
  wx[0] =
      ((0.090909 * (xx - 2.0) - 0.215311) * (xx - 2.0) + 0.124402) * (xx - 2.0);
  xx = xx - 1.0;
  wx[1] = ((-0.545455 * (xx - 1.0) + 1.291866) * (xx - 1.0) - 0.746411) *
          (xx - 1.0);
  xx = xx - 1.0;
  ...
  xx = xx + 1.0;
  wx[4] = ((-0.545455 * (xx - 1.0) + 1.291866) * (xx - 1.0) - 0.746411) *
          (xx - 1.0);
  xx = xx + 1.0;
  wx[5] =
      ((0.090909 * (xx - 2.0) - 0.215311) * (xx - 2.0) + 0.124402) * (xx - 2.0);
...
//!!! PAZI, TOLE NE DELA CISTO PRAV ???   belina se siri
//!!! zaenkrat sem dodal fudge factor...
int interpSP6_b32(unsigned char *sl, int w, int h, float x, float y,
                  unsigned char *v) {
  int i, b, j, m, n;
  float pp, p[6], wx[6], wy[6], xx;
...
//	x,y tocka, za katero izracuna interpolirano vrednost
//	*v interpolirana vrednost
int interpSC16_b(unsigned char *sl, int w, int h, float x, float y,
                 unsigned char *v) {
  int i, j, m, n;
  ...
  for (i = 7; i >= 0; i--) {
    x1 = xx * PI;
    wy[7 - i] =
        (x1 != 0) ? (sin(x1) / (x1)) * (sin(x1 * 0.125) / (x1 * 0.125)) : 1.0;
    xxx = (float)(2 * i + 1) - xx;
    x1 = xxx * PI;
    wy[8 + i] =
        (x1 != 0) ? (sin(x1) / (x1)) * (sin(x1 * 0.125) / (x1 * 0.125)) : 1.0;
...
// truncated sinc "lanczos" 16x16 interpolacija
// za byte (char) vrednosti  v packed color 32 bitnem formatu
int interpSC16_b32(unsigned char *sl, int w, int h, float x, float y,
                   unsigned char *v) {
  int i, j, m, b, n;
  float pp, p[16], wx[16], wy[16], xx, xxx, x1;
  float PI = 3.141592654;
