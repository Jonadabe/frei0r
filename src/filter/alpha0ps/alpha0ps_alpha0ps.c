...
void shave_alpha(float *sl, float *ab, int w, int h) {
  ...
  for (i = 1; i < h - 1; i++) {
    p = i * w + 1;
    for (j = 1; j < w - 1; j++) {
      m = sl[p - 1] + sl[p + 1] + sl[p - w] + sl[p + w] + sl[p - 1 - w] +
          sl[p + 1 + w] + sl[p + 1 - w] + sl[p - 1 + w];
      m = m * 0.125;
      ab[p] = (sl[p] < m) ? sl[p] : m;
      p++;
    }
...
//----------------------------------------------------------
void grow_alpha(float *al, float *ab, int w, int h, int mode) {
  int i, j, p;
  float m, md;
...
//----------------------------------------------------------
void shrink_alpha(float *al, float *ab, int w, int h, int mode) {
  int i, j, p;
  float m, md;
...
void blur_alpha(inst *in, float *falpha) {
  ...
  fibe2o_f(falpha, in->w, in->h, in->a1, in->a2, in->rd1, in->rd2, in->rs1,
           in->rs2, in->rc1, in->rc2, 1);
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
  ...
  in->f = 0.05;
  in->q = 0.55; // blur
  calcab_lp1(in->f, in->q, &in->a0, &in->a1, &in->a2, &in->b0, &in->b1,
             &in->b2);
  in->a1 = in->a1 / in->a0;
  in->a2 = in->a2 / in->a0;
...
void f0r_set_param_value(f0r_instance_t instance, f0r_param_t parm,
  ...
  float iir2f[] = {0.475,  0.39,    0.325,   0.26,   0.21,   0.155,   0.112,
                   ...
                   0.0093, 0.00725, 0.00505, 0.0033, 0.0025};
