...
void calcab_lp1(float f, float q, float *a0, float *a1, float *a2, float *b0,
  ...
  *a1 = -2.0 * b;
  *a2 = 1.0 - a;
  // printf("a=%9.6f %9.6f %9.6f   b=%9.6f %9.6f
  // %9.6f\n",*a0,*a1,*a2,*b0,*b1,*b2);
}
...
void fibe2o_f(float s[], int w, int h, float a1, float a2, float rd1, float rd2,
              float rs1, float rs2, float rc1, float rc2, int ec) {
  ...
  for (j = 0; j < avg; j++) // prvih avg vrstic tja in nazaj
  {
    ...
    rep1 =
        (s[jww - 1] + s[jww - 2]) * 0.5 * rs1 + (s[jww - 1] - s[jww - 2]) * rd1;
    rep2 =
        (s[jww - 1] + s[jww - 2]) * 0.5 * rs2 + (s[jww - 1] - s[jww - 2]) * rd2;
  ...
  for (j = avg; j < h; j++) // po vrsticah tja, nazaj in dol
  {
    ...
    {                            // nazaj
      ...
      // dol
      s[jw + i + 2] =
          s[jw + i + 2] - a1 * s[jw - w + i + 2] - a2 * s[jw - w - w + i + 2];
    }
  ...
  for (j = 0; j < w; j++) // po stolpcih
  {
    ...
    rep1 =
        (s[j + h1w] + s[j + h2w]) * 0.5 * rs1 + (s[j + h1w] - s[j + h2w]) * rd1;
    rep2 =
        (s[j + h1w] + s[j + h2w]) * 0.5 * rs2 + (s[j + h1w] - s[j + h2w]) * rd2;
