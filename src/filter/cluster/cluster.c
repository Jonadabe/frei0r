...
void f0r_deinit() { /* no initialization required */
}
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
  ...
  for (k = 0; k < MAXNUM; k++) {
    ...
    const unsigned char *src2 =
        (unsigned char *)(&inframe[x + inst->width * y]);
...
void f0r_destruct(f0r_instance_t instance) { free(instance); }
...
float find_dist(int r1, int g1, int b1, int x1, int y1, int r2, int g2, int b2,
                int x2, int y2, float max_space_dist,
                float dist_weight) //, float color_weight)
{
  /// make this a define?
  ...
  return sqrtf((1.0 - dist_weight) * color_dist * color_dist +
               dist_weight * space_dist * space_dist);
}
...
void f0r_update(f0r_instance_t instance, double time, const uint32_t *inframe,
  ...
  float max_space_dist =
      sqrtf(inst->width * inst->width + inst->height * inst->height);
  ...
  */
  ...
  for (y = 0; y < inst->height; ++y) {
    for (x = 0; x < inst->width; ++x) {
      ...
      for (k = 0; k < inst->num; k++) {
        ...
        float kdist = find_dist(src2[0], src2[1], src2[2], x, y, cc->r, cc->g,
                                cc->b, cc->x, cc->y, max_space_dist,
                                inst->dist_weight); //, inst->color_weight);
  ...
  for (k = 0; k < inst->num; k++) {
    ...
    if (cc->numpix > 0) {
      ...
      // (unsigned int)cc->b); printf("%g, %g %g %g\n", cc->numpix, cc->aggr_r,
      // cc->aggr_g, cc->aggr_b);
    }
