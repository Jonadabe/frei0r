 ...
 * source code; if  not, write to: Free Software Foundation, Inc., 675 Mass Ave,
...
Baltan::~Baltan() { free(planebuf); }

void Baltan::update(double time, uint32_t *out, const uint32_t *in) {
  int i, cf;
  ...
  for (i = 0; i < pixels; i++) {
    dst[i] = (src[i] & 0xFF000000) |
             (planetable[cf][i] + planetable[cf + STRIDE][i] +
              planetable[cf + STRIDE2][i] + planetable[cf + STRIDE3][i]);
