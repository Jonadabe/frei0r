...
void f0r_get_plugin_info(f0r_plugin_info_t *info) {
  ...
  info->num_params = 11;
  info->explanation = "Draws a gradient on top of image. Filter is given "
                      "gradient start and end points, colors and opacities.";
}
...
void f0r_get_param_info(f0r_param_info_t *info, int param_index) {
  ...
  case 0:
    ...
    info->explanation =
        "Linear or radial gradient"; // Accepted values: 'gradient_linear' and
                                     // 'gradient_radial
    break;
  case 1:
  ...
  case 9:
    info->name = "offset";
    info->type = F0R_PARAM_DOUBLE;
    info->explanation = "Position of first color in the line connecting "
                        "gradient ends, really useful only for radial gradient";
    break;
  case 10:
    info->name = "blend mode";
    info->type = F0R_PARAM_STRING;
    info->explanation =
        "Blend mode used to compose gradient on image. Accepted values: "
        "'normal', 'add', 'saturate', 'multiply', 'screen', 'overlay', "
        "'darken', 'lighten', 'colordodge', 'colorburn', 'hardlight', "
        "'softlight', 'difference', 'exclusion', 'hslhue', 'hslsaturation', "
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
  cairo_gradient_instance_t *inst =
      (cairo_gradient_instance_t *)calloc(1, sizeof(*inst));
  inst->width = width;
  inst->height = height;
...
void draw_gradient(cairo_gradient_instance_t *inst, unsigned char *dst,
                   const unsigned char *src, double time) {
  int stride = cairo_format_stride_for_width(CAIRO_FORMAT_ARGB32, inst->width);
  cairo_surface_t *surface = cairo_image_surface_create_for_data(
      dst, CAIRO_FORMAT_ARGB32, inst->width, inst->height, stride);
  cairo_t *cr = cairo_create(surface);

  cairo_surface_t *src_image = cairo_image_surface_create_for_data(
      (unsigned char *)src, CAIRO_FORMAT_ARGB32, inst->width, inst->height,
      stride);
  ...
  if (strcmp(inst->pattern, GRADIENT_RADIAL) == 0) {
    ...
    pat = cairo_pattern_create_radial(sx * inst->width, sy * inst->height, 0.0,
                                      sx * inst->width, sy * inst->height,
                                      distance);
  } else {
    pat = cairo_pattern_create_linear(sx * inst->width, sy * inst->height,
                                      ex * inst->width, ey * inst->height);
  }

  freior_cairo_set_color_stop_rgba_LITTLE_ENDIAN(
      pat, 1.0, inst->start_color.r, inst->start_color.g, inst->start_color.b,
      inst->start_opacity);
  freior_cairo_set_color_stop_rgba_LITTLE_ENDIAN(
      pat, inst->offset, inst->end_color.r, inst->end_color.g,
      inst->end_color.b, inst->end_opacity);
