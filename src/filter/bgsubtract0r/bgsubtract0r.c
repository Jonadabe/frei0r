...
void f0r_get_plugin_info(f0r_plugin_info_t *bgsubtract0r_info) {
  ...
  bgsubtract0r_info->explanation =
      "Bluescreen the background of a static video.";
}

f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
  bgsubtract0r_instance_t *inst =
      (bgsubtract0r_instance_t *)calloc(1, sizeof(*inst));
  inst->width = width;
  inst->height = height;
  inst->denoise = 1;
  inst->blur = 0;
...
void f0r_update(f0r_instance_t instance, double time, const uint32_t *inframe,
  ...
  if (!inst->reference) {
    ...
    for (i = 0; i < len; i++)
      mask[i] =
          (dst(inst->reference[i], inframe[i]) > inst->threshold) ? 0xff : 0;
  }
  ...
  if (inst->denoise)
    for (j = 1; j < height - 1; j++)
      for (i = 1; i < width - 1; i++) {
        n = (mask[width * j + i - 1] + mask[width * j + i + 1] +
             mask[width * (j - 1) + i] + mask[width * (j + 1) + i] +
             mask[width * (j - 1) + i - 1] + mask[width * (j - 1) + i + 1] +
             mask[width * (j + 1) + i - 1] + mask[width * (j + 1) + i + 1]) /
            0xff;
        if (mask[width * j + i]) {
          if (n <= 2)
