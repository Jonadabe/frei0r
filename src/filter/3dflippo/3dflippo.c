...
void f0r_get_param_info(f0r_param_info_t *info, int param_index) {
  ...
  case 9:
    ...
    info->type = F0R_PARAM_BOOL;
    info->explanation = "Mask for frame transposition is not blanked, so a "
                        "trace of old transpositions is maintained";
    break;
  case 10:
    info->name = "Fill with image or black";
    info->type = F0R_PARAM_BOOL;
    info->explanation = "If true, pixels that are not transposed are black, "
                        "otherwise, they are copied with the original";
    break;
  }
}
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height) {
  ...
  inst->flip[0] = inst->flip[1] = inst->flip[2] = inst->rate[0] =
      inst->rate[1] = inst->rate[2] = 0.5;
...
void f0r_update(f0r_instance_t instance, double time, const uint32_t *inframe,
  ...
  if (inst->rate[0] != 0.5 || inst->rate[1] != 0.5 || inst->rate[2] != 0.5 ||
      inst->mustrecompute) {
