...
void calcab_lp1(float f, float q, float *a0, float *a1, float *a2, float *b0,
                float *b1, float *b2) {
  float a, b;
...
// gain ni kompenziran
void rep(float v1, float v2, float c, float *i1, float *i2, int n, float a1,
         float a2) {
  int i;
  float lb[8192];
...
// simplified indexes
void fibe1o_8(const uint32_t *inframe, uint32_t *outframe, float_rgba *s, int w,
              int h, float a, int ec) {
  int i, j;
  ...
  if (ec != 0) {
    for (i = 0; i < w; i++) // po stolpcih
    {
      ...
      outframe[p + i] = ((uint32_t)s[p + i].r & 0xFF) +
                        (((uint32_t)s[p + i].g & 0xFF) << 8) +
                        (((uint32_t)s[p + i].b & 0xFF) << 16);
    }
  } else {
    for (j = 0; j < w; j++) // po stolpcih
    {
      ...
      s[j + p].g = g4b * s[j + p].g;
      s[j + p].b = g4b * s[j + p].b;
      outframe[p + j] = ((uint32_t)s[p + j].r & 0xFF) +
                        (((uint32_t)s[p + j].g & 0xFF) << 8) +
                        (((uint32_t)s[p + j].b & 0xFF) << 16);
    }
  }
...
// optimized for speed
void fibe2o_8(const uint32_t *inframe, uint32_t *outframe, float_rgba s[],
              int w, int h, float a1, float a2, float rd1, float rd2, float rs1,
              float rs2, float rc1, float rc2, int ec) {
  ...
  for (j = 0; j < avg; j++) // prvih avg vrstic tja in nazaj
  {
    ...
    cg = 0.0;
    cb = 0.0;
    if (ec != 0) { // edge comp (popvprecje prvih)
      for (i = 0; i < avg; i++) {
    ...
    {
      s[jw + i].r =
          g4 * s[jw + i].r - a1 * s[jw + i - 1].r - a2 * s[jw + i - 2].r;
      s[jw + i].g =
          g4 * s[jw + i].g - a1 * s[jw + i - 1].g - a2 * s[jw + i - 2].g;
      s[jw + i].b =
          g4 * s[jw + i].b - a1 * s[jw + i - 1].b - a2 * s[jw + i - 2].b;
    }
    ...
    rep1.r = (s[jww - 1].r + s[jww - 2].r) * 0.5 * rs1 +
             (s[jww - 1].r - s[jww - 2].r) * rd1;
    rep1.g = (s[jww - 1].g + s[jww - 2].g) * 0.5 * rs1 +
             (s[jww - 1].g - s[jww - 2].g) * rd1;
    rep1.b = (s[jww - 1].b + s[jww - 2].b) * 0.5 * rs1 +
             (s[jww - 1].b - s[jww - 2].b) * rd1;
    rep2.r = (s[jww - 1].r + s[jww - 2].r) * 0.5 * rs2 +
             (s[jww - 1].r - s[jww - 2].r) * rd2;
    rep2.g = (s[jww - 1].g + s[jww - 2].g) * 0.5 * rs2 +
             (s[jww - 1].g - s[jww - 2].g) * rd2;
    rep2.b = (s[jww - 1].b + s[jww - 2].b) * 0.5 * rs2 +
             (s[jww - 1].b - s[jww - 2].b) * rd2;
  ...
  for (j = avg; j < h; j++) // po vrsticah tja, nazaj in dol
  {
    ...
    {                            // nazaj
      ...
      s[jw + i].b = s[jw + i].b - a1 * s[jw + i + 1].b - a2 * s[jw + i + 2].b;
      // dol
      s[jw + i + 2].r = s[jw + i + 2].r - a1 * s[jw - w + i + 2].r -
                        a2 * s[jw - w - w + i + 2].r;
      s[jw + i + 2].g = s[jw + i + 2].g - a1 * s[jw - w + i + 2].g -
                        a2 * s[jw - w - w + i + 2].g;
      s[jw + i + 2].b = s[jw + i + 2].b - a1 * s[jw - w + i + 2].b -
                        a2 * s[jw - w - w + i + 2].b;
    }
  ...
  for (j = 0; j < w; j++) // po stolpcih
  {
    ...
    rep1.r = (s[j + h1w].r + s[j + h2w].r) * 0.5 * rs1 +
             (s[j + h1w].r - s[j + h2w].r) * rd1;
    rep1.g = (s[j + h1w].g + s[j + h2w].g) * 0.5 * rs1 +
             (s[j + h1w].g - s[j + h2w].g) * rd1;
    rep1.b = (s[j + h1w].b + s[j + h2w].b) * 0.5 * rs1 +
             (s[j + h1w].b - s[j + h2w].b) * rd1;
    rep2.r = (s[j + h1w].r + s[j + h2w].r) * 0.5 * rs2 +
             (s[j + h1w].r - s[j + h2w].r) * rd2;
    rep2.g = (s[j + h1w].g + s[j + h2w].g) * 0.5 * rs2 +
             (s[j + h1w].g - s[j + h2w].g) * rd2;
    rep2.b = (s[j + h1w].b + s[j + h2w].b) * 0.5 * rs2 +
             (s[j + h1w].b - s[j + h2w].b) * rd2;
    ...
    if (s[j + h1w].b < 0.0)
      s[j + h1w].b = 0.0;
    outframe[j + h1w] = ((uint32_t)s[j + h1w].r & 0xFF) +
                        (((uint32_t)s[j + h1w].g & 0xFF) << 8) +
                        (((uint32_t)s[j + h1w].b & 0xFF) << 16);
    ...
    if (s[j + h2w].b < 0.0)
      s[j + h2w].b = 0.0;
    outframe[j + h2w] = ((uint32_t)s[j + h2w].r & 0xFF) +
                        (((uint32_t)s[j + h2w].g & 0xFF) << 8) +
                        (((uint32_t)s[j + h2w].b & 0xFF) << 16);
  }
  ...
  for (i = h - 3; i >= 0; i--) // gor
  {
    ...
    for (j = 0; j < w; j++) {
      ...
      if (s[j + iw].b < 0.0)
        s[j + iw].b = 0.0;
      outframe[j + iw] = ((uint32_t)s[j + iw].r & 0xFF) +
                         (((uint32_t)s[j + iw].g & 0xFF) << 8) +
                         (((uint32_t)s[j + iw].b & 0xFF) << 16);
    }
  }
}
...
void fibe3_8(const uint32_t *inframe, uint32_t *outframe, float_rgba s[], int w,
             int h, float a1, float a2, float a3, int ec) {
  float cr, cg, cb, g, g4;
  int i, j;
  ...
  for (j = 0; j < h; j++) // po vrsticah
  {
    ...
    {
      lb[i].r = g4 * s[j * w + i].r - a1 * lb[i - 1].r - a2 * lb[i - 2].r -
                a3 * lb[i - 3].r;
      lb[i].g = g4 * s[j * w + i].g - a1 * lb[i - 1].g - a2 * lb[i - 2].g -
                a3 * lb[i - 3].g;
      lb[i].b = g4 * s[j * w + i].b - a1 * lb[i - 1].b - a2 * lb[i - 2].b -
                a3 * lb[i - 3].b;
    }
    ...
    lb[w + cez - 3].r =
        lb[w + cez - 3].r - a1 * lb[w + cez - 2].r - a2 * lb[w + cez - 1].r;
    lb[w + cez - 3].g =
        lb[w + cez - 3].g - a1 * lb[w + cez - 2].g - a2 * lb[w + cez - 1].g;
    lb[w + cez - 3].b =
        lb[w + cez - 3].b - a1 * lb[w + cez - 2].b - a2 * lb[w + cez - 1].b;
    for (i = (w + cez - 4); i >= w; i--) {
      lb[i].r =
          lb[i].r - a1 * lb[i + 1].r - a2 * lb[i + 2].r - a3 * lb[i + 3].r;
      lb[i].g =
          lb[i].g - a1 * lb[i + 1].g - a2 * lb[i + 2].g - a3 * lb[i + 3].g;
      lb[i].b =
          lb[i].b - a1 * lb[i + 1].b - a2 * lb[i + 2].b - a3 * lb[i + 3].b;
    }

    s[j * w + w - 1].r =
        lb[w - 1].r - a1 * lb[w].r - a2 * lb[w + 1].r - a3 * lb[w + 2].r;
    s[j * w + w - 1].g =
        lb[w - 1].g - a1 * lb[w].g - a2 * lb[w + 1].g - a3 * lb[w + 2].g;
    s[j * w + w - 1].b =
        lb[w - 1].b - a1 * lb[w].b - a2 * lb[w + 1].b - a3 * lb[w + 2].b;
    s[j * w + w - 2].r =
        lb[w - 2].r - a1 * s[j * w + w - 1].r - a2 * lb[w].r - a3 * lb[w + 1].r;
    s[j * w + w - 2].g =
        lb[w - 2].g - a1 * s[j * w + w - 1].g - a2 * lb[w].g - a3 * lb[w + 1].g;
    s[j * w + w - 2].b =
        lb[w - 2].b - a1 * s[j * w + w - 1].b - a2 * lb[w].b - a3 * lb[w + 1].b;
    s[j * w + w - 3].r = lb[w - 3].r - a1 * s[j * w + w - 2].r -
                         a2 * s[j * w + w - 1].r - a3 * lb[w].r;
    s[j * w + w - 3].g = lb[w - 3].g - a1 * s[j * w + w - 2].g -
                         a2 * s[j * w + w - 1].g - a3 * lb[w].g;
    s[j * w + w - 3].b = lb[w - 3].b - a1 * s[j * w + w - 2].b -
                         a2 * s[j * w + w - 1].b - a3 * lb[w].b;

    for (i = w - 4; i >= 0; i--) // nazaj
    {
      s[j * w + i].r = lb[i].r - a1 * s[j * w + i + 1].r -
                       a2 * s[j * w + i + 2].r - a3 * s[j * w + i + 3].r;
      s[j * w + i].g = lb[i].g - a1 * s[j * w + i + 1].g -
                       a2 * s[j * w + i + 2].g - a3 * s[j * w + i + 3].g;
      s[j * w + i].b = lb[i].b - a1 * s[j * w + i + 1].b -
                       a2 * s[j * w + i + 2].b - a3 * s[j * w + i + 3].b;
    }
  } // po vrsticah
  ...
  for (j = 0; j < w; j++) // po stolpcih
  {
    ...
    for (i = 3; i < h; i++) // dol
    {
      lb[i].r = s[j + w * i].r - a1 * lb[i - 1].r - a2 * lb[i - 2].r -
                a3 * lb[i - 3].r;
      lb[i].g = s[j + w * i].g - a1 * lb[i - 1].g - a2 * lb[i - 2].g -
                a3 * lb[i - 3].g;
      lb[i].b = s[j + w * i].b - a1 * lb[i - 1].b - a2 * lb[i - 2].b -
                a3 * lb[i - 3].b;
    }
    ...
    lb[h + cez - 3].r =
        lb[h + cez - 3].r - a1 * lb[h + cez - 2].r - a2 * lb[h + cez - 1].r;
    lb[h + cez - 3].g =
        lb[h + cez - 3].g - a1 * lb[h + cez - 2].g - a2 * lb[h + cez - 1].g;
    lb[h + cez - 3].b =
        lb[h + cez - 3].b - a1 * lb[h + cez - 2].b - a2 * lb[h + cez - 1].b;
    for (i = (h + cez - 4); i >= h; i--) {
      lb[i].r =
          lb[i].r - a1 * lb[i + 1].r - a2 * lb[i + 2].r - a3 * lb[i + 3].r;
      lb[i].g =
    ...
    s[j + (h - 1) * w].r =
        lb[h - 1].r - a1 * lb[h].r - a2 * lb[h + 1].r - a3 * lb[h + 2].r;
    s[j + (h - 1) * w].g =
        lb[h - 1].g - a1 * lb[h].g - a2 * lb[h + 1].g - a3 * lb[h + 2].g;
    s[j + (h - 1) * w].b =
        lb[h - 1].b - a1 * lb[h].b - a2 * lb[h + 1].b - a3 * lb[h + 2].b;
    s[j + (h - 2) * w].r = lb[h - 2].r - a1 * s[j + (h - 1) * w].r -
                           a2 * lb[h].r - a3 * lb[h + 1].r;
    s[j + (h - 2) * w].g = lb[h - 2].g - a1 * s[j + (h - 1) * w].g -
                           a2 * lb[h].g - a3 * lb[h + 1].g;
    s[j + (h - 2) * w].b = lb[h - 2].b - a1 * s[j + (h - 1) * w].b -
                           a2 * lb[h].b - a3 * lb[h + 1].b;
    s[j + (h - 3) * w].r = lb[h - 3].r - a1 * s[j + (h - 2) * w].r -
                           a2 * s[j + (h - 1) * w].r - a3 * lb[h].r;
    s[j + (h - 3) * w].g = lb[h - 3].g - a1 * s[j + (h - 2) * w].g -
                           a2 * s[j + (h - 1) * w].g - a3 * lb[h].g;
    s[j + (h - 3) * w].b = lb[h - 3].b - a1 * s[j + (h - 2) * w].b -
                           a2 * s[j + (h - 1) * w].b - a3 * lb[h].b;
    ...
    for (i = h - 4; i >= 0; i--) // gor
    {
      ...
      s[j + w * i].b = lb[i].b - a1 * s[j + w * (i + 1)].b -
                       a2 * s[j + w * (i + 2)].b - a3 * s[j + w * (i + 3)].b;
      outframe[j + w * i] = ((uint32_t)s[j + w * i].r & 0xFF) +
                            (((uint32_t)s[j + w * i].g & 0xFF) << 8) +
                            (((uint32_t)s[j + w * i].b & 0xFF) << 16);
    }
  } // po stolpcih
